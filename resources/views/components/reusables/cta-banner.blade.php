...
<section class="aquarius-cta-banner">
    ...
    <div class="aquarius-container-content relative z-10 text-center py-16 lg:py-20">
        <h2 class="text-2xl lg:text-4xl font-bold text-white mb-4">{{ $title }}</h2>
        <p class="text-neutral-300 text-base lg:text-lg mb-8 max-w-xl mx-auto">{{ $body }}</p>
        <a href="{{ $href ?? route('contact-page') }}"
            class="inline-flex items-center gap-2 bg-white text-primary-900 font-semibold px-7 py-3.5 rounded-xl hover:bg-primary-50 transition-colors shadow-lg">
            {{ $label }}
